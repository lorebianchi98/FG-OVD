...
"""
import sys
sys.path.append('/raid/homes/lorenzo.bianchi/trial/FG-OVD/NoctOWL/scenic/scenic/projects/owl_vit/big_vision')
...
import tqdm
from transformers import OwlViTForObjectDetection, Owlv2ForObjectDetection
...
flags.DEFINE_string(
    'noctowl_weights', None,
    'Path of the NoctOWL weights')
...
def load_noctowl_weights(variables, hf_weights):
    if 'v2' not in hf_weights:
        hf_model = OwlViTForObjectDetection.from_pretrained(hf_weights)
    else:
        hf_model = Owlv2ForObjectDetection.from_pretrained(hf_weights)
    variables['params']['class_head']['Dense_0']['kernel'] = hf_model.class_head.dense0.weight
    variables['params']['class_head']['Dense_0']['bias'] = hf_model.class_head.dense0.bias
    return variables
...
def get_predictions(config: ml_collections.ConfigDict,
  ...
  # Loading NoctOWL weights if requested
  if FLAGS.noctowl_weights is not None:
    variables = load_noctowl_weights(variables, FLAGS.noctowl_weights)
    logging.info(f"Loaded NoctOWL weights {FLAGS.noctowl_weights}")
